...
# --- WPÓŁRZĘDNE DLA GRUPY ---
# Musimy zapisać antenę i karolka osobno, żeby je potem połączyć w grupę
...
# --- 3. BAZA WASZYCH PÓL (Zaktualizowana) ---
POLA = {
    "Czterdziestka (40 ha)": [
    ...
    "Koło anteny": [coords_anteny],
    # NOWA GRUPA: łączymy antenę i karolka w jedną listę
    "Koło anteny i karolka (grupa)": [coords_anteny, coords_karolka],
    "Za Jurkiem": [
...
# Tworzymy listę opcji do menu: "WSZYSTKIE POLA" na samej górze
...
# Wyznaczamy jakie poligony mamy wyświetlić i czy robimy analizę
if wybrane_nazwa == "WSZYSTKIE POLA (Widok ogólny)":
    wszystkie_poligony_list = [poly for p_list in POLA.values() for poly in p_list]
    analiza_dostepna = False
else:
    wszystkie_poligony_list = POLA[wybrane_nazwa]
...
# Zbieramy wszystkie współrzędne z wybranej listy poligonów
...
# Powiększamy delikatnie kadr o 10%, żeby pola nie stykały się z krawędzią
...
# Zapisujemy dokładne granice zdjęcia do przeliczania pikseli
    ...
    with st.spinner('Przeszukuję archiwum satelity w poszukiwaniu bezchmurnego dnia...'):
        try:
            ...
            if not dobre_zdjecia:
                st.warning(f"Niestety, między {start_dt} a {end_dt} nad wybranym obszarem wisiały same grube chmury. Wybierz inną datę.")
                st.stop()
            ...
            if fetched_data:
                ...
                # --- PRZYGOTOWANIE MASKI (DLA WIELU KAWAŁKÓW LUB CAŁOŚCI) ---
                # Tworzymy pustą czarną maskę
                maska_globalna = np.zeros((600, 600), dtype=bool)
                
                # Przygotowujemy siatkę punktów (pikseli) do sprawdzenia
                x_grid, y_grid = np.meshgrid(np.arange(600), np.arange(600))
                points = np.vstack((x_grid.flatten(), y_grid.flatten())).T
                
                # Lista na współrzędne pikselowe obramowań do narysowania
                sciezki_do_rysowania = []
                
                # Przechodzimy przez każdy poligon na liście do wyświetlenia
                for poligon in wszystkie_poligony_list:
                    # Przeliczamy współrzędne geograficzne na piksele zdjęcia (600x600)
                    pixel_coords = []
                    for lon, lat in poligon:
                        px = (lon - img_min_x) / (img_max_x - img_min_x) * 600
                        # Pyplot rysuje Y od góry, więc musimy obrócić os
                        py = 600 - ((lat - img_min_y) / (img_max_y - img_min_y) * 600) 
                        pixel_coords.append([px, py])
                        
                    # Tworzymy ścieżkę dla tego konkretnego poligonu
                    path = Path(pixel_coords)
                    
                    # Sprawdzamy, które punkty siatki leżą wewnątrz tej ścieżki
                    poly_mask = path.contains_points(points).reshape(600, 600)
                    
                    # Łączymy maski za pomocą operacji logicznej OR (|)
                    # Piksel będzie biały, jeśli leży wewnątrz CHOCIAŻ JEDNEGO poligonu
                    maska_globalna = maska_globalna | poly_mask
                    
                    # Zapisujemy przeliczone piksele do narysowania żółtej linii
                ...
                if not analiza_dostepna:
                    # Widok ogólny - po prostu zdjęcie RGB z obramowaniem
                    ...
                    # Widok konkretnego pola/grupy - tło czarno-białe (grayscale)
                    ...
                    # Nakładamy kolorową warstwę analizy TYLKO wewnątrz maski
                    if typ_analizy == "Kondycja roślin (NDVI)":
                        mapa_warstwa = raw_data[:, :, 3].copy()
                        mapa_warstwa[~maska_globalna] = np.nan # Ukrywamy wszystko poza maską
                        ...
                        info_tekst = f"💡 {wybrane_nazwa}: Intensywny zielony to mocna wegetacja. Czerwony to problem lub goła ziemia."
                        ...
                        info_tekst = f"💧 {wybrane_nazwa}: Ciemny turkus to woda w komórkach/błoto. Brąz to postępująca susza."

                    else: # ZONING (Strefy)
                        # Bierzemy NDVI pod maską
                        ndvi_raw = raw_data[:, :, 3].copy()
                        ndvi_raw[~maska_globalna] = np.nan
                        
                        # Liczymy percentyle dla całego obszaru pod maską
                        ...
                        strefy[~maska_globalna] = np.nan # Ukrywamy tło
                        
                        # Paleta: Czerwony -> Żółty -> Zielony
                        cmap_strefy = mcolors.ListedColormap(['#d73027', '#fee08b', '#1a9850'])
                        im = ax.imshow(strefy, cmap=cmap_strefy, alpha=0.85, interpolation='nearest') # Kanciaste dla traktora
                        
                        # Pasek legendy z opisem stref
                        cbar = plt.colorbar(im, fraction=0.046, pad=0.04, ticks=[1, 2, 3])
                        cbar.set_ticklabels(['Strefa 1\n(Słaby wigor)', 'Strefa 2\n(Średniak)', 'Strefa 3\n(Mocny wigor)'])
                        info_tekst = f"🚜 {wybrane_nazwa}: Strefy VRA dla rozsiewacza. UWAGA: Stosuj tylko, gdy na NDVI widać wyraźne różnice!"

                # Rysujemy żółte lub czarne obramowania dla wszystkich poligonów na zdjęciu
                ...
                # Formatowanie finalnej grafiki
                ax.axis('off') # Ukrywamy osie (piksele/współrzędne)
                fig.patch.set_facecolor('white') # Białe tło wokoło
                
                st.pyplot(fig) # Wyświetlamy mapę w aplikacji
                st.info(info_tekst) # Dodajemy tekst informacyjny poniżej

        except Exception as e:
            st.error(f"Wystąpił błąd podczas analizy. Być może satelita nie ma zdjęcia dla tej kombinacji obszaru i daty. Błąd: {e}")